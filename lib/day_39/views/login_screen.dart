...
import 'package:ppkd_b_5/day_39/views/register_screen.dart';
import 'package:ppkd_b_5/extension/navigator.dart';

class LoginFirebaseScreenDay39 extends StatefulWidget {
  const LoginFirebaseScreenDay39({super.key});

  @override
  State<LoginFirebaseScreenDay39> createState() =>
      _LoginFirebaseScreenDay39State();
}

class _LoginFirebaseScreenDay39State extends State<LoginFirebaseScreenDay39> {
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      String? message;
                      try {
                        final login = await FirebaseService.loginUser(
                          email: emailController.text,
                          password: passwordController.text,
                        );
                        if (login != null) {
                          log(login.toJson());
                          PreferenceHandler().storingToken(login.uid!);
                          PreferenceHandler().storingIsLogin(true);
                          context.pushReplacement(DrawerGlobal());
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text("Akun belum terdaftar")),
                          );
                        }
                      } catch (e) {
                        log(e.toString());
                        ...
                        children: [
                          ...
                          TextSpan(
                            recognizer: TapGestureRecognizer()
                              ..onTap = () {
                                context.push(const RegisterFirebaseScreen());
                              },
                            text: "Daftar",
                            style: TextStyle(
